...
void RosInterface::run() {
  ...
  std::thread t_slam([&]() {
    ...
    ros::Time ros_stamp;
    while (ros::ok()) {
      timestamp = zed->GetStereoFrame(&img_left, &img_right); //ns
      cv::imshow("zed_left", img_left);
      cv::waitKey(1);
      // zed_mask_lock.lock();
      // zedLeftMaskL = zedLeftMask.clone();
      // zed_mask_lock.unlock();
      if(timestamp > zed->qImuBuf.back().t) continue;
      std::vector<ORB_SLAM3::IMU::Point> vImuMeas;
      zed->mBufMutex.lock();
      if(!zed->qImuBuf.empty())
      {
        // Load imu measurements from buffer
        vImuMeas.clear();
        while(!zed->qImuBuf.empty() && zed->qImuBuf.front().t<=timestamp)
        {
          double t = NANO2SEC(zed->qImuBuf.front().t);
          vImuMeas.push_back(ORB_SLAM3::IMU::Point(zed->qImuBuf.front().a,zed->qImuBuf.front().w,t));
          zed->qImuBuf.pop();
        }
      }
      zed->mBufMutex.unlock();
      // cv::remap(img_left,img_left,M1l,M2l,cv::INTER_LINEAR);
      // cv::remap(img_right,img_right,M1r,M2r,cv::INTER_LINEAR);
      // my_sys->feed_stereo_IMU(img_left, img_right, timestamp, vImuMeas);
      cv::Mat Tcw = my_sys->SLAM_->TrackStereo(img_left,img_right,timestamp/1e9,vImuMeas);

      ros_stamp.sec = timestamp / 1e9;
      ros_stamp.nsec = (timestamp % 1000000000);
